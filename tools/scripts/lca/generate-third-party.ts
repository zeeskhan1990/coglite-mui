...
const defaultLicenses = {
  ...
  "(ofl-1.1 and mit)": fs
    .readFileSync(path.join(defaultLicenseRoot, "ofl-1.1-and-mit.txt"))
    .toString(),
}
...
function getLicenseContent(dependency, license) {
  if (!license.content) {
    ...
    if (licenseType in defaultLicenses) {
      ...
      console.warn(
        `Repo ${dependency.name} doesn't have a license file` +
...
function checkNoticeUpToDate(notices: string) {
  ...
  if (existingNotices === notices) {
    ...
    console.error(
      "ThirdPartyNotice.txt is not up to date." +
...
function run(options: ThirdPartyNoticeOptions = {}) {
  ...
  const dependencies = depenencyNames
    .map(dep => {
